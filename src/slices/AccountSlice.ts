...
interface IUserAccountDetails {
    balances: {
        ...
        fhud: string;
        usdb: string;
        ohmThirtyDayStaked: number;
        ohmThirtyDayClaimable: number;
        ohmThirtyDayTotal: number;
        lastStakedBlock: number;
        noFeeBlocks: number;
    ...
    async ({ networkId, address }: IBaseAddressAsyncThunk, { dispatch }) => {
        ...
        if (addresses[networkId].OHM_ADDRESS || addresses[networkId].SOHM_ADDRESS || addresses[networkId].WSOHM_ADDRESS) {
            ...
            if (networkId === 250) {
                ...
                [fhudBalance, fhudAllowance, usdbBalance] = await Promise.all([
                    fhudContract.balanceOf(address),
                    fhudContract.allowance(address, addresses[networkId].USDB_MINTER),
                    usdbContract.balanceOf(address)
                ]);
            }
        ...
        let fsohmBalance = 0;
        let wsOhmstakeAllowance = 0;
        let lastStakedBlock = 0;
        let noFeeBlocks = 0;
        let stake30DayTotal = 0;
        let totalHoldings = 0;
        let lpStaked = 0;
        ...
        if (addresses[networkId].STAKINGSTAKING_ADDRESS && addresses[networkId].WSOHM_ADDRESS) {
            const wsohmContract = new ethers.Contract(addresses[networkId].WSOHM_ADDRESS as string, wsOHM, provider);
            const stakingstakingContract = new ethers.Contract(addresses[networkId].STAKINGSTAKING_ADDRESS as string, StakingStaking, provider);

            let userInfo30DayStake = null;
            let userBalance30DayStake = [];
            [
                wsOhmstakeAllowance,
                userInfo30DayStake,
                userBalance30DayStake,
                needToClaim,
                noFeeBlocks,
                totalHoldings
            ] = await Promise.all([
                wsohmContract.allowance(address, addresses[networkId].STAKINGSTAKING_ADDRESS),
                stakingstakingContract.userInfo(address),
                stakingstakingContract.userBalance(address),
                stakingstakingContract.needToClaim(address),
                stakingstakingContract.noFeeBlocks(),
                stakingstakingContract.totalHoldings()
            ]);

            lastStakedBlock = userInfo30DayStake.lastStakeBlockNumber;
            stake30DayTotal += userBalance30DayStake[0];
            ohmThirtyDayClaimable += userBalance30DayStake[1];
        }
        ...
        return {
            balances: {
                ...
                ohmThirtyDayStaked: ethers.utils.formatUnits(lastStakedBlock, 18),
                ohmThirtyDayClaimable: ethers.utils.formatUnits(ohmThirtyDayClaimable, 18),
                ohmThirtyDayTotal: ethers.utils.formatUnits(stake30DayTotal, 18),
                lastStakedBlock: lastStakedBlock,
                noFeeBlocks: noFeeBlocks,
                bridge: ethers.utils.formatUnits(bridgeTokenBalance, "gwei"),
                totalHoldings: ethers.utils.formatUnits(totalHoldings, 18),
            },
            staking: {
                ohmStake: +stakeAllowance,
                ohmUnstake: +unstakeAllowance,
                fhudAllowance: +fhudAllowance,
                wsohmStake: +wsOhmstakeAllowance,
