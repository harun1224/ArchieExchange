...
export async function getMarketPrice(networkId : NetworkId) {
	...
	return reserveDecimals.token1Decimals === 9 ? reserve0 / reserve1 : reserve1 / reserve0;
}

const validCacheTime = 60 * 1000;

export const saveBannerStatus = (status: string) => {
	if (window.localStorage) {
		window.localStorage.setItem("bannerStatus", status);
	}
};

export const getSavedBannerStatus = () => {
	return window.localStorage && window.localStorage.getItem("bannerStatus");
}

export const bannerStatus = () => {
	return getSavedBannerStatus() !== "hide";
};
...
export function formatPercent(number = 0, precision = 0) {
	if (number > 10**13) {
...
export function getRunway(sOHM: number, rfv: number, rebase: number): number {
	if (sOHM > 0 && rfv > 0 && rebase > 0) {
		...
		// (1125.38 * 1.72733670271 + 7273.58 * 2.912621359223301) / (7273.58 + 1125.38) = 2.75380459539
		// (1.72733670271 + 2.912621359223301) / 2 = 2.31997903097
		return (Math.log(treasury_runway) / Math.log(1 + nextEpochRebase_number)) / 2.75380459539;
	}
	return 0;
}
