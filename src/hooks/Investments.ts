...
function useInvestments() {
  ...
  useEffect(() => {
    ...
    const investments = Object.values(largestHoldings.reduce((value: any, object) => {
      if (value[object.displayName]) {
        value[object.displayName].treasuryBalance += object.treasuryBalance;
        value[object.displayName].assetBalance += object.assetBalance;
      } else {
        value[object.displayName] = { ...object };
      }
      return value;
    }, {}));
    setInvestments(investments as Investment[]);
