import {
  ...
  function BestRoute1(props) {
    ...
    return (
      <Box position="relative" minHeight="330px" p="15px" display="flex" flexDirection="column" alignItems="center"
           justifyContent="center" className="dash-border bestRoute">
        { props?.routeLoading ? <ReactLoading type="spinningBubbles" color="#000000" /> : (
          <>
                    ...
                    return (
                      <Box width="80px" borderRadius="5px" bgcolor={`${!darkmodeStatus ? '#C9CDD2' : '#46485f'}`} key={ `route_${ index }` }
                        ...
                        <Box mt="10px" display="flex" flexDirection="column" justifyContent="center" alignItems="center">
                          <Typography noWrap style={ { fontSize: "10px" } } className={`${darkmodeStatus ? 'text-white' : 'text-black'} w-full`}>Fee
                            ≈ { formatCurrency(feeCalculator(swap.fee, props?.metaData), 3) }</Typography>
                          <Typography noWrap style={ { fontSize: "10px" } } className={`${darkmodeStatus ? 'text-white' : 'text-black'} w-full`}>Time
  ...
  export default memo(BestRoute1);